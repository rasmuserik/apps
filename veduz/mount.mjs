...
async function mount_loop() {
  ...
  for (const id of mount.keys()) {
    ...
    let server_update = local_changes.map((o) => ({
      ...o,
      path: serverPath + "/" + o.path,
    }));
    ...
    if (serverChanges.length > 0) {
      serverChanges = serverChanges.map((o) => {
        o = { ...o };
        if (o.path.startsWith(serverPath)) {
          o.path = o.path.replace(serverPath, "");
        }
        return o;
      });
      ...
      await update(
        remoteData.path(),
        ({ cur }) => cur.apply_changes(serverChanges),
      );
